...
- Working tree is dirty with uncommitted 2026-07-09 security fixes (`server/index.js`, `routes/auth|leads|timesheet`, `services/email.js`, 5 `src/` files).
...
|---|---|---|
| `/` | `app/page.js` | Home (Hero, EpoxyInfo, Warranty, FlakeCarousel, Gallery, Testimonials, ContactForm) |
| `/commercial`, `/garage-makeover`, `/patios`, `/radon`, `/polished-concrete`, `/careers`, `/thank-you`, `/privacy`, `/terms`, `/snake` | `app/<route>/page.js` | 1:1 ports |
| `/colors` | `app/colors/page.js` | Keeps code-split via `next/dynamic` (517-color catalog) |
| `/epoxy-flooring-albuquerque`, `-santa-fe`, `-rio-rancho` | `app/epoxy-flooring-[city]/page.js` | One dynamic route + `generateStaticParams` fed by `src/locations.js`; unknown city → 404 |
| `/admin/*`, `/signin` etc. | `app/admin/*` (+ sign-in route) | `'use client'`; AdminRoute JWT gate reused; noindex via metadata `robots` + existing nginx X-Robots map |
| unknown URLs | `app/not-found.js` | Branded 404 (port of NotFound.jsx) with real HTTP 404 status natively |

Shared structure:

- `app/layout.js` (server component): Header, Footer, StickyCallButton, base metadata (site-wide OG image, og:site_name/locale/type, fallback title template), base JSON-LD @graph (`#business` LocalBusiness + 3 Service nodes), GA4 `next/script` loader wrapped in a small client component that reproduces today's route gating (no gtag on `/admin*`/`/sign*`).
- Per-page `metadata` exports replace every react-helmet block: unique title, description, canonical (`https://www.nextlevelepoxynm.com<path>`), og:title/description/url. Next's metadata merging is the single source of truth — this natively eliminates the duplicate-tag class of bugs behind the past de-indexing incident.
...
- `app/sitemap.js` generates the sitemap from a single route manifest (static pages + `locations.js`); `app/robots.js` generates robots.txt (Allow /, Disallow /admin/, sitemap pointer). The hand-maintained `public/sitemap.xml` is deleted. This manifest is the extension point phase 2's blog slugs plug into.
- The Snake easter egg and ThankYou page port unchanged. `/thank-you` keeps its noindex if present today (parity harness confirms either way).

## 6. SEO parity harness (cutover gate)

New permanent tooling: `scripts/seo-parity.js` (Node, no puppeteer — plain fetch + HTML parsing).

- **Snapshot mode:** crawls every route on `https://www.nextlevelepoxynm.com` and records per route: HTTP status, title, meta description, canonical, robots meta, og:title/description/url/image, H1 text, JSON-LD (parsed, key fields), redirect behavior (http→https, non-www→www).
- **Compare mode:** runs the same crawl against the staging target (`http://127.0.0.1:3000` on EC2, or local `next start`) and diffs against the snapshot. Exit non-zero on any difference not in an explicit allowlist file (e.g. sitemap lastmod values, intentional improvements).
...
**deploy.sh v2** (laptop): `npm run build` → rsync `.next/standalone/`, `.next/static/`, `public/` to EC2 (`--partial`, retry-once on failure) → `pm2 startOrRestart` `nextlevel-web` → health-check `curl 127.0.0.1:3000` → done. Express deploy path (rsync `server/` + `pm2 restart nextlevel-api`) unchanged.
...
5. nginx edit per incident procedure: `location /` → `proxy_pass http://127.0.0.1:3000` (keep `/api/*` proxy, all security headers, www-canonical, X-Robots map; drop the static-root try_files/error_page bits). `nginx -t` → reload.
