...
- **Existing usage rows referencing a soft-deleted item.** Still resolve via JOIN, so day cards continue to display the original name after the item is removed from the active list. PUT and DELETE on `/api/inventory/usage/:id` intentionally **do not** re-check `deleted_at` on the parent item — this lets historical usage be corrected (e.g. fix a typo'd quantity from last week) even if the item has since been removed from the active inventory list.
